...
def quantum_compute_v2(alice_data, bob_data):
    alice_qpairs = alice_data["pairings"]
    alice_groupcodes = alice_data["groupings"]
    ...
    correct_guesses = []
    total_measurements = len(alice_qpairs)
    errors = 0
    ...
    for i in range(total_measurements):
        ...
        if verify_circuit(qc):
            ...
            errors += 1
            print("Bob's guess was incorrect")

    alice_data["correct_measurements"] = correct_guesses
    bob_data["correct_measurements"] = correct_guesses

    # Calculate QBER
    qber = errors / total_measurements if total_measurements > 0 else 0

    return alice_data, bob_data, qber


def t22_qkd_protocol_v2(key_length):
    alice_data = {
        "pairings": generate_random_pairings(key_length),
        "groupings": generate_random_groupings(key_length)
    }
    bob_data = {
        "pairings": generate_random_pairings(key_length),
        "groupings": generate_random_groupings(key_length)
    ...
    alice_result, bob_result, qber = quantum_compute_v2(alice_data, bob_data)

    alice_correct, bob_correct = get_correct_measurements(alice_result, bob_result)

    alice_key = generate_code(alice_correct["groupings"])
    bob_key = generate_code(bob_correct["groupings"])
